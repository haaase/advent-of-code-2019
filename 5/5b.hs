...
interp prog input output pc
    ...
    | opcd == 5 = case (val1 /= 0) of
        True  -> interp prog input output val2 -- jump
        False -> interp prog input output (pc + 3) -- don't jump
    | opcd == 6 = case (val1 == 0) of
        True  -> interp prog input output val2 -- jump
        False -> interp prog input output (pc + 3) -- don't jump
    | opcd == 7 = case (val1 < val2) of
        True  -> interp (update arg3 1 prog) input output (pc + 4)
        False -> interp (update arg3 0 prog) input output (pc + 4)
    | opcd == 8 = case (val1 == val2) of
        True  -> interp (update arg3 1 prog) input output (pc + 4)
        False -> interp (update arg3 0 prog) input output (pc + 4)
